...
const ICONS = {
  plinko: (<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="5" r="2.5"/><line x1="6" y1="22" x2="8" y2="12"/><line x1="18" y1="22" x2="16" y2="12"/><line x1="8" y1="12" x2="16" y2="12"/></svg>),
  ...
  info:   (<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="8.01"/><line x1="12" y1="12" x2="12" y2="16"/></svg>),
  crash:  (<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/><polyline points="17 6 23 6 23 12"/></svg>),
  blackjack: (<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="3" width="14" height="18" rx="2"/><path d="M16 8h4a2 2 0 0 1 0 4h-4"/><path d="M16 16h4a2 2 0 0 0 0-4h-4"/></svg>),
};
        ...
        <thead>
          <tr>
            {table.columns.map((col, i) => (
              <th key={i} className="pf-rt__th">{col}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {table.rows.map((row, ri) => (
            <tr key={ri} className="pf-rt__row">
              {row.map((cell, ci) => (
                <td key={ci} className="pf-rt__td">
    ...
    <>
      {terms.map((item, i) => (
        <p key={i} style={{ fontSize: '0.88rem' }}>
          <strong>{item.term}</strong><br />{item.definition}
        </p>
      ))}
    </>
  );
...
function ChecklistGroups({ groups }) {
  ...
  return (
    <>
      {groups.map((group, gi) => (
        <div key={gi}>
          <p style={{ fontSize: '0.82rem', fontWeight: 700, margin: '0.8rem 0 0.3rem' }}>{group.heading}</p>
...
function RefSection({ section }) {
  const body = (
    <>
      {section.intro           && <p style={{ fontSize: '0.88rem', padding: '0 1.2rem' }}>{section.intro}</p>}
      {section.content         && <p style={{ fontSize: '0.88rem', padding: '0 1.2rem' }}>{section.content}</p>}
      {section.steps           && <div style={{ padding: '0 1.2rem' }}><RefSteps steps={section.steps} /></div>}
      {section.bullets         && <div style={{ padding: '0 1.2rem' }}><RefBullets bullets={section.bullets} /></div>}
      {section.terms           && <div style={{ padding: '0 1.2rem' }}><RefTerms terms={section.terms} /></div>}
      {section.table           && <RefTable table={section.table} />}
      {section.checklistGroups && <ChecklistGroups groups={section.checklistGroups} />}
      {section.note            && <div className="pf-note" style={{ margin: '0.4rem 1.2rem' }}>{section.note}</div>}
    </>
  );
  ...
  return (
    <div className="sub">
      <div className="sub-hdr" style={{ padding: '0 1.2rem' }}>
      ...
      {references.map((ref, i) => (
        <details key={i} className="pf-ref-panel">
          <summary className="pf-ref-panel__summary">
            ...
            {ref.count && <span className="pf-ref-panel__count">{ref.count}</span>}
            <span className="pf-ref-panel__arrow">▶</span>
          </summary>
          <div className="pf-ref-panel__body">
            {ref.intro   && <p style={{ fontSize: '0.88rem', padding: '0 1.2rem' }}>{ref.intro}</p>}
            {ref.bullets && <div style={{ padding: '0 1.2rem' }}><RefBullets bullets={ref.bullets} /></div>}
